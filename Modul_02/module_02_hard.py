...
def divisibility(a, b, n_):              # Функция проверки делимости
    if n_ % (a + b) == 0:
        str_ = str(a) + str(b)
        return str_


def kod(n_):                            # Функция по выработке массива и формирования строки "Код"
    mas_ = [[0, 0]]
    len_mas = 1
    for i in range(1, n_ // 2 + 1):
        for j in range(1, n_):
            if i + j <= n_ and i != j:  # отсортировка дублей типа [2 2] и [2 2]
                flag1 = False
                for k in range(len_mas):  # отсортировка дублей типа [1 2] и [2 1]
                    if mas_[k][1] == i and mas_[k][0] == j:
                        flag1 = True
                        break
                if not flag1:
                    mas_.append([i, j])
    ...
    for i in range(len(mas_)):
        str_ = divisibility(mas_[i][0], mas_[i][1], n_)
        if str_ is not None:
            s = s + str_
    return s
